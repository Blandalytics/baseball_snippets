...
st.write('Find me [@Blandalytics](https://bsky.app/profile/blandalytics.pitcherlist.com) and check out the data at [Baseball Savant](https://baseballsavant.mlb.com/leaderboard/bat-tracking)')
...
st.write('NOTE: Bat Speed measurements are [capped at 88mph](https://x.com/tangotiger/status/1838630623373615459) (for the public data)')
pl_white = '#FEFEFE'
...
with col1:
    swing_threshold = st.number_input(f'Min # of Swings (in all situations):',
                                      ...
                                      step=5, 
                                      value=200 if year=='2024' else 100)
with col2:
...
def swing_heatmap(df,hitter,base_stat,handedness):
    ...
    stat_dict = {
        heatmap_stat_dict[base_stat][0]:[heatmap_stat_dict[base_stat][1],swing_data[base_stat].mean()/(20 if base_stat=='swing_acceleration' else 15)]
    }
    
    bandwidth = 0.2# if base_stat == 'squared_up_frac' else 0.5
    ...
    for stat in stat_dict.keys():
        ...
        scale_text = ax.annotate(f' is {stat_value_dict[stat][0]} than Lg Avg, ', fontsize=10,
                                 xycoords=scale_text, xy=(1, 0), va="bottom",color="k")
        scale_text = ax.annotate('Red', xycoords=scale_text, xy=(1, 0), va="bottom", fontsize=10,
                                 color=sns.color_palette('vlag',n_colors=1000)[-1], fontweight='bold')
        scale_text = ax.annotate(f' is {stat_value_dict[stat][1]}', xycoords=scale_text, fontsize=10,
