...
def load_data():
    df = pd.read_parquet('https://huggingface.co/datasets/blandalytics/nfbc_draft_data/resolve/main/nfbc_adp_data.parquet')
...
with col1:
    ...
    new_title = f'<p style="color:#72CBFD; font-size: 20px;">Biggest risers since {adp_start_date.strftime('%-m/%-d/%y')}{pos_text}</p>'
    st.markdown(new_title, unsafe_allow_html=True)
...
def plot_draft_data(df,player,start_date):
  ...
  pl_ax = fig.add_axes([0.44,-0.05,0.2,0.22], anchor='S', zorder=1)
  pl_ax.imshow(logo)
  pl_ax.axis('off')
